...
import { Failure } from "./genesis";
import { Params } from "./params";

export const protobufPackage = "neutron.interchainadapter.contractmanager";
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("neutron.interchainadapter.contractmanager.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  AddressFailures(request: QueryFailuresRequest): Promise<QueryFailuresResponse> {
    const data = QueryFailuresRequest.encode(request).finish();
    const promise = this.rpc.request("neutron.interchainadapter.contractmanager.Query", "AddressFailures", data);
    return promise.then((data) => QueryFailuresResponse.decode(new _m0.Reader(data)));
  }

  Failures(request: QueryFailuresRequest): Promise<QueryFailuresResponse> {
    const data = QueryFailuresRequest.encode(request).finish();
    const promise = this.rpc.request("neutron.interchainadapter.contractmanager.Query", "Failures", data);
    return promise.then((data) => QueryFailuresResponse.decode(new _m0.Reader(data)));
  }
}
