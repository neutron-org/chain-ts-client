...
export { MsgSubmitQueryResult, MsgRemoveInterchainQueryRequest, MsgRegisterInterchainQuery };
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgSubmitQueryResult({ value }: msgSubmitQueryResultParams): EncodeObject {
			try {
				return { typeUrl: "/neutron.interchainadapter.interchainqueries.MsgSubmitQueryResult", value: MsgSubmitQueryResult.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgRemoveInterchainQueryRequest({ value }: msgRemoveInterchainQueryRequestParams): EncodeObject {
			try {
				return { typeUrl: "/neutron.interchainadapter.interchainqueries.MsgRemoveInterchainQueryRequest", value: MsgRemoveInterchainQueryRequest.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgRegisterInterchainQuery({ value }: msgRegisterInterchainQueryParams): EncodeObject {
			try {
				return { typeUrl: "/neutron.interchainadapter.interchainqueries.MsgRegisterInterchainQuery", value: MsgRegisterInterchainQuery.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			NeutronInterchainadapterInterchainqueries: new SDKModule(test)
		},
		registry: msgTypes
  }
}
